...
function createShader2el(gl, vshEl, fshEl) {
  return createShader2(gl, $(vshEl).text(), $(fshEl).text());
}
...
function loadShaders() {
  if (Object.keys(shadersCache).length)
    return;
  var fs = require('fs');
  shadersCache['#vsh-basic'] = fs.readFileSync(__dirname + '/webgl/vsh-basic.vert', 'utf8');
  shadersCache['#fsh-simple-texture'] = fs.readFileSync(__dirname + '/webgl/fsh-simple-texture.frag', 'utf8');
  shadersCache['#fsh-lanczos-1d-covolve-horizontal'] = fs.readFileSync(__dirname + '/webgl/fsh-lanczos-1d-covolve-horizontal.frag', 'utf8');
  shadersCache['#fsh-lanczos-1d-covolve-vertical'] = fs.readFileSync(__dirname + '/webgl/fsh-lanczos-1d-covolve-vertical.frag', 'utf8');
}
...
function webglProcessResize(from, gl, options) {
  ...
  function convolve(texUnit0, texWidth, texHeight, texUnit, fsh, winSize, width, height) {
    ...
    setAttributeValues(gl, program, 'a_position', vec2Rectangle(0, 0, outsize.width, outsize.height), { elementSize: 2 });
  ...
  setAttributeValues(gl, program, 'a_texCoord', [ 0, 0, 1, 0, 0, 1, 0, 1, 1, 0, 1, 1 ], { elementSize: 2 });
  setAttributeValues(gl, program, 'a_position', vec2Rectangle(0, 0, gl.canvas.width, gl.canvas.height), { elementSize: 2 });
