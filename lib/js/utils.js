...
var PIXEL_EPSILON = 1e-5; // precision for coordinates in pixels

function fixPrecision(x) {
  var nearest = Math.round(x);
  return (Math.abs(x - nearest) < PIXEL_EPSILON) ? nearest : x;
}
...
module.exports.createRegions = function createRegions(options) {
  ...
  var innerTileWidth = Math.floor(fixPrecision(options.srcTileSize * scaleX)) - 2 * options.destTileBorder;
  var innerTileHeight = Math.floor(fixPrecision(options.srcTileSize * scaleY)) - 2 * options.destTileBorder;
  ...
  for (innerY = 0; innerY < options.toHeight; innerY += innerTileHeight) {
    for (innerX = 0; innerX < options.toWidth; innerX += innerTileWidth) {
      ...
      tile = {
        ...
        width: Math.ceil(fixPrecision(toTileWidth / scaleX)),
        height: Math.ceil(fixPrecision(toTileHeight / scaleY))
      };
