...
var unpack_custom_filter = require('./resize_filter_info_custom').unpack;
...
module.exports = function resizeFilterGen(filter, srcSize, destSize, scale, offset) {
  ...
  // Normalize filter data to { win, fn }
  if (typeof filter === 'string') f_info = FILTER_INFO.filter[filter];
  else f_info = unpack_custom_filter(filter);

  var filterFunction = f_info.fn;

  var scaleInverted = 1.0 / scale;
  var scaleClamped  = Math.min(1.0, scale); // For upscale

  // Filter window (averaging interval), scaled to src image
  var srcWindow = f_info.win / scaleClamped;
