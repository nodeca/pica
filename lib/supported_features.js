...
const ORIENTED_JPEG_BASE64 =
  ...
  'AAAACf/EABwQAAIDAQADAAAAAAAAAAAAAAECAwQFCAAGB//aAAgBAQAAPwA9MnpvpLAyq' +
  'WFhdB/Ss7Nzq8dSnTqe2X4YK0EahY4o41lCoiqAoUAAAADz/9k=';
...
function check_offscreen_canvas() {
  ...
  try {
    let canvas = new OffscreenCanvas(2, 1);
    let ctx = canvas.getContext('2d');
...
function check_bug_canvas_orientation_region_async() {
  return Promise.resolve().then(() => {
    let canOffscreenCanvas = check_offscreen_canvas() &&
                             typeof createImageBitmap !== 'undefined' &&
                             typeof Blob !== 'undefined' &&
                             typeof atob !== 'undefined';

    if (canOffscreenCanvas) {
      let binary = atob(ORIENTED_JPEG_BASE64);
...
function check_cib_resize_async() {
  return Promise.resolve().then(() => {
    ...
    .then(bitmap => {
      let status = bitmap.width === DST_SIZE && !!bitmap.close;
...
function get_supported_features() {
  ...
  let bugCanvasOrientationRegion = check_bug_canvas_orientation_region_async()
    .then(result => { features.bug_canvas_orientation_region = result; })
    .catch(() => {});

  let bugImageBitmapOrientationRegion = check_bug_image_bitmap_orientation_region_async()
    .then(result => { features.bug_image_bitmap_orientation_region = result; })
    .catch(() => {});

  let cibResize = check_cib_resize_async()
