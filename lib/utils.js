...
function isCloseDistance(originalValue, currentValue){

  return Math.abs(originalValue - currentValue) < 5;
}
...
module.exports.can_use_canvas = function can_use_canvas(createCanvas) {
  ...
  try {
    ...
    const originalData = [12, 23, 34, 255, 45, 56, 67, 255];
    originalData.forEach((value, index) => d.data[index] = value);
    ctx.putImageData(d, 0, 0);
    d = null;

    d = ctx.getImageData(0, 0, 2, 1);

    if (originalData.every((originalValue, index) => isCloseDistance(originalValue, d.data[index]))) {
      usable = true;
    }
  } catch (err) {}
